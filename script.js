    ...
    },
    {
            ...
            filteredCars.forEach(car => {  
                const carCard = `  
                    <div class="bg-white rounded shadow p-4">  
                        <img src="${car.images[0]}" alt="Car" class="w-full h-32 object-cover object-center rounded">  
                        <h3 class="font-bold text-lg mt-2">${car.price} ${car.currency}</h3>  
                        <p>${car.brand} ${car.model}</p>  
                        <p>${car.year}, ${car.odometer} ${car.odometerUnit}</p>  
                        <p>${car.city}</p>  
                        <p>${car.banType}</p>  
                        <p>${car.credit ? 'Kredit' : 'Kredit yoxdur'}</p>  
                        <p>${car.barter ? 'Barter mümkündür' : 'Barter yoxdur'}</p>  
                    </div>  
                `;  
        ...
        function populateFilters() {
            ...
            brandSelect.innerHTML = '<option value="">Marka</option>' + brands.map(brand => 
                `<option value="${brand}">${brand}</option>`).join('');

            modelSelect.innerHTML = '<option value="">Model</option>' + models.map(model => 
                `<option value="${model}">${model}</option>`).join('');
        }

        function filterCars() {  
            const brand = document.getElementById('brandFilter').value;  
            const model = document.getElementById('modelFilter').value;  
            const newUsed = document.getElementById('newUsedFilter').value;  
            const minPrice = parseInt(document.getElementById('minPrice').value) || 0;  
            const maxPrice = parseInt(document.getElementById('maxPrice').value) || Infinity;  
            const currency = document.getElementById('currencyFilter').value;  
            const credit = document.getElementById('creditFilter').checked;  
            const barter = document.getElementById('barterFilter').checked;  
            const city = document.getElementById('cityFilter').value;  

            const filteredCars = cars.filter(car => {
                const priceInAZN = car.currency === 'USD' ? car.price * 1.7 : car.price;
                
                return (!brand || car.brand === brand) &&
                    (!model || car.model === model) &&
                    (!newUsed || 
                        (newUsed === 'Yeni' && car.odometer === 0) || 
                        (newUsed === 'Sürülmüş' && car.odometer > 0)) &&
                    (priceInAZN >= minPrice && priceInAZN <= maxPrice) &&
        ...
        document.getElementById('filterButton').onclick = filterCars;  

        displayCars(cars);
        populateFilters();  
        
        function dropMenu(buttonId, menuId) {
        ...
        dropMenu('accessoriesButton', 'accessoriesMenu');
        dropMenu('rentalButton', 'rentalMenu');